...
```bash
qmd query "question"              # Auto-expand + rerank (slowest; best recall)
qmd query $'lex: X\nvec: Y'       # Structured
qmd query $'expand: question'     # Explicit expand
qmd query --json --explain "q"    # Show score traces (RRF + rerank blend)
qmd search "keywords"             # BM25 only (no LLM — fastest)
qmd vsearch "question"            # Vector-only (no expand, no rerank)
qmd get "#abc123"                 # By docid
qmd multi-get "journals/2026-*.md" -l 40  # Batch pull snippets by glob
qmd multi-get notes/foo.md,notes/bar.md   # Comma-separated list, preserves order
```

### Speed flags (check hardware — `qmd status`)

LLM reranking is the slow part. On CPU-only machines or large indexes (>5k docs), `qmd query` can take 10–30s. Levers:

| Flag | Effect |
|---|---|
| `--no-rerank` | Skip LLM rerank, use RRF fusion of lex+vec scores. Much faster on CPU. |
| `-C, --candidate-limit <n>` | Rerank only top N candidates (default 40). Lower = faster. |
| `qmd search` instead of `qmd query` | Pure BM25, no LLM anywhere. Fastest path for exact strings. |
| `qmd vsearch` | Single vector search, no expand or rerank. |

**Rule of thumb:** start with `qmd search` for keywords, `qmd query --no-rerank` for semantic-with-speed, full `qmd query` only when recall really matters.
